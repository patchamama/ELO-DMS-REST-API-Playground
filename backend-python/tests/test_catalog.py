...
def test_topics_load_and_have_required_fields():
    ...
    for t in topics:
        ...
        assert "elo.New" in t.snippets.go
...
def test_lab_perms_flag_loads():
    by_id = {t.id: t for t in load_topics("en")}
    assert by_id["lab.user-folder-permissions"].lab_perms is True
    assert by_id["connection.login"].lab_perms is False


def test_lab_recent_and_workflow_flags_load():
    by_id = {t.id: t for t in load_topics("en")}
