...
final class EloClient {
  ...
  public function call(string $method, array $body = []): mixed {
    ...
    $raw = file_get_contents($url, false, $ctx); if ($raw === false) throw new RuntimeException('IX request failed'); $envelope = json_decode($raw, true, 512, JSON_THROW_ON_ERROR); return $envelope['result'] ?? new stdClass();
  }
}
