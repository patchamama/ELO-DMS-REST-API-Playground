...
$result = $elo->call('checkoutUsers', json_decode('{"ids":{"$expression":"[u[\'id\'] for u in users]"},"checkoutUsersZ":{"bset":"513"}}', true, 512, JSON_THROW_ON_ERROR));
...
echo json_encode($result, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), PHP_EOL;
$result = $elo->call('checkoutSord', json_decode('{"objId":{"$expression":"str(folder_id)"},"editInfoZ":{"bset":"1","sordZ":{"bset":{"$expression":"ALL"}}}}', true, 512, JSON_THROW_ON_ERROR));
echo json_encode($result, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), PHP_EOL;
